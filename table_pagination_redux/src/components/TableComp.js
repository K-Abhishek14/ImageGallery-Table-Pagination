...
class TableComp extends Component {
	...
	render() {
		const { classes,emp } = this.props;
		const {  page, rowsPerPage } = this.state
		console.log("TABLE RENDER", emp);
		return (
			<Paper className={classes.root}>
				<Table>
					...
					<TableBody>
					{emp !== undefined && emp.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(item => (
...
export default TableComp = withStyles(styles, { withTheme: true })(TableComp)
